...
const withTiming = async (
  fn: () => Promise<Omit<DiagnosticResult, 'latencyMs'>>,
...
export const checkDbConnectivity = () =>
  withTiming(async () => {
    ...
    try {
      ...
      return {
        ...
        status: 'fail',
        message: err.message,
        errorCategory: 'UnknownDbError',
        suggestedFix: 'Verify DB credentials.',
      };
    }
  });

export const checkDbWritePermission = () =>
  withTiming(async () => {
    ...
    try {
      ...
      if (deleteError) {
        return {
          name,
          status: 'warn',
          message: `Insert succeeded, but delete failed: ${deleteError.message}`,
          errorCategory: 'DeleteError',
          suggestedFix: 'Check RLS delete policies on diagnostics_log.',
        };
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          category = 'AuthError';
          fix = `Check ${name} API key.`;
        }
        if (response.status === 404) {
          category = 'ModelNotFound';
          fix = `Check if model ${model} is valid.`;
        }
        if (response.status === 429) {
          category = 'RateLimited';
          fix = 'Check provider quota/billing.';
        }
      ...
      return {
        ...
        suggestedFix: 'Check network connectivity or DNS.',
      };
    }
  });
};

export const checkPrimaryAi = () =>
  testProviderCall(
    'Primary AI Provider',
    config.primaryAiApiKey || '',
    config.primaryAiBaseUrl,
    config.primaryAiModel,
    'Primary AI Provider',
  );

export const checkSecondaryAi = () =>
  testProviderCall(
    'Secondary AI Provider',
    config.secondaryAiApiKey || '',
    config.secondaryAiBaseUrl,
    config.secondaryAiModel,
    'Secondary AI Provider',
  );

export const checkTertiaryGroq = () =>
  testProviderCall(
...
export const checkFallbackChainEndToEnd = () =>
  withTiming(async () => {
    ...
    try {
      ...
      try {
        ...
        if (res && res.success === false) {
          // Assuming success might be false if error
        ...
        return {
          name,
          status: 'pass',
          message: 'Fallback chain successfully routed around the invalid primary key.',
        };
      } catch (err: any) {
        config.solverAiApiKey = originalKey;
        config.useNewAiArchitecture = originalUseNew;
...
export const checkVectorSearch = () =>
  withTiming(async () => {
    ...
    try {
      ...
      if (!triggered429) {
        return {
          name,
          status: 'warn',
          message: 'Rate limiter did not trigger after 150 requests.',
          errorCategory: 'RateLimiterNotTripping',
          suggestedFix: 'Verify rate limiting middleware configuration.',
        };
      }
      ...
      return {
        name,
        status: 'pass',
        message: 'Rate limiter successfully triggered at expected threshold.',
      };
    } catch (err: any) {
      return {
        name,
        status: 'warn',
        message: 'Could not test rate limiter: ' + err.message,
        errorCategory: 'TestError',
        suggestedFix: 'Server may not be running locally on the expected port.',
      };
    }
  });

export const checkCacheSystem = () =>
  withTiming(async () => {
    ...
    try {
      ...
      if (read !== testVal) {
        return {
          name,
          status: 'fail',
          message: 'Cache read value did not match set value.',
          errorCategory: 'CacheIntegrityError',
          suggestedFix: 'Check LRU cache configuration.',
        };
      }
    ...
    try {
      try {
        ...
        return {
          name,
          status: 'fail',
          message: 'verifyToken incorrectly accepted a malformed token.',
          errorCategory: 'AuthVulnerability',
          suggestedFix: 'Check Clerk configuration and verifyToken implementation.',
        };
      } catch (err) {
        // Expected failure
