...
export const Navbar: React.FC<NavbarProps> = React.memo(
  ({ soundEnabled = true, onOpenSettings, isCollapsed, onToggleCollapse, isTeacherMode }) => {
    const { user } = useUser();
    ...
    return (
      <nav
        className={`fixed bottom-0 left-0 right-0 z-40 md:bottom-0 md:top-0 ${isCollapsed ? 'md:w-[72px]' : 'md:w-[240px]'} md:h-screen md:rounded-none bg-white dark:bg-[#111113] border-t border-zinc-200 dark:border-white/[0.06] md:border-t-0 md:border-r py-2 px-3 md:py-5 md:px-3 premium-transition flex md:flex-col justify-between`}
      >
        ...
        <div className="hidden md:flex flex-col gap-6 relative">
          ...
          <Link
            to="/"
            className={`flex items-center gap-2.5 px-2 cursor-pointer group ${isCollapsed ? 'justify-center' : ''}`}
          >
            <m.div
        ...
        <div className="hidden md:flex flex-col gap-1">
          ...
          <div
            className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-lg bg-zinc-50 dark:bg-white/[0.03] border border-zinc-200/60 dark:border-white/[0.04] mt-1 group hover:border-zinc-300 dark:hover:border-white/[0.08] premium-transition ${isCollapsed ? 'justify-center px-2' : ''}`}
          >
