...
export const ChatMessageItem = React.memo(
  ({
      ...
      return (
        ...
        >
          {copied ? (
            ...
            <Copy className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-50 opacity-60 hover:opacity-100" />
          )}
        </button>
      );
    };
    ...
    const renderAssistantMessage = () => {
              ...
              <div className="relative">
                ...
                >
                  {msg.is_pinned ? (
                    ...
                    <Pin className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-50 opacity-50 hover:opacity-100" />
                  )}
                </button>
                <DualAiResponseView
                  data={parsed}
                ...
                />
                <div className="mt-2 bg-white/80 dark:bg-[#18181B]/80 border border-black/5 dark:border-white/5 rounded-2xl px-4 py-1 shadow-sm backdrop-blur-md">
                  <MessageActionBar
      ...
      return (
        <div
          className={`bg-[#FAFAFA] dark:bg-[#18181B] border border-black/5 dark:border-white/5 shadow-sm rounded-2xl rounded-tl-sm px-5 py-4 text-[14px] text-gray-800 dark:text-gray-200 leading-relaxed prose prose-sm max-w-none prose-p:leading-relaxed overflow-visible relative group ${msg.is_pinned ? 'ring-2 ring-amber-400 dark:ring-amber-500/50' : ''}`}
        >
                ...
                code({ node, className, children, ...props }: any) {
                  ...
                  return !isInline ? (
                    <div className="rounded-xl overflow-hidden my-4 border border-black/10 dark:border-white/10 shadow-sm">
                      ...
                      <pre
                        className="p-4 bg-gray-50 dark:bg-[#09090B] text-sm overflow-x-auto text-black dark:text-white m-0"
                        {...props}
                      >
                    ...
                    <code
                      className="bg-black/5 dark:bg-white/10 px-1.5 py-0.5 rounded-md text-[0.9em] font-mono text-[#F43F5E] dark:text-[#FB7185]"
                      {...props}
                    >
                      {children}
                    </code>
      ...
      >
        {msg.role === 'user' ? (
          <div className="flex justify-end items-end gap-2 relative group/user-msg">
            <div
              className={`bg-[#09090B] dark:bg-[#FAFAFA] text-white dark:text-black shadow-sm rounded-2xl rounded-br-sm px-4 py-3 max-w-full ${msg.is_pinned ? 'ring-2 ring-amber-400 dark:ring-amber-500/50' : ''}`}
            >
              {isEditing ? (
                <div className="flex flex-col gap-2 min-w-[250px]">
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="px-3 py-1 text-xs rounded-md bg-white/20 dark:bg-black/10 hover:bg-white/30 dark:hover:bg-black/20 transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => {
                        setIsEditing(false);
                        if (onEditMessage && editValue.trim() !== msg.content)
                          onEditMessage(msg.id, editValue);
                      }}
                      className="px-3 py-1 text-xs rounded-md bg-[#2563EB] text-white hover:bg-[#1D4ED8] transition-colors"
                    >
                      Save & Submit
                    </button>
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  {msg.image_url && (
                    <img
                      src={msg.image_url}
                      alt="User attachment"
                      className="rounded-lg max-w-[200px] h-auto object-cover border border-white/20 dark:border-black/20"
                    />
                  )}
