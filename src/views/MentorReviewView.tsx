  ...
  const handleResolve = async (reviewId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setQueue((prev) =>
          prev.map((item) =>
            item.id === reviewId ? { ...item, status: 'resolved', resolution_notes: notes } : item,
          ),
        );
      } else {
      ...
      <div className="flex flex-col items-center justify-center h-full min-h-[50vh]">
        <div className="text-center text-zinc-500">
          ...
          <p className="mt-2 text-sm">
            Please enable Teacher Mode in settings to access this page.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 dark:text-zinc-400 mt-1 text-sm">
            Review flagged student interactions and provide guidance.
          </p>
        </div>
        <button
          onClick={fetchQueue}
          ...
          {queue.length === 0 && !loading && (
            ...
            >
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                There are no pending items in the review queue.
              </p>
            </m.div>
          )}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {item.status === 'resolved' && (
                  <span className="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-md">
                    Resolved
                  </span>
                )}
                {item.status !== 'resolved' && (
                  <span className="px-2 py-1 text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 rounded-md">
                    Pending Review
                  </span>
                )}
              </div>
              ...
              {item.status === 'resolved' ? (
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setResolutionInput((prev) => ({ ...prev, [item.id]: e.target.value }))
                    }
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      {resolvingId === item.id ? (
                        ...
                        <>
                          <Check className="w-4 h-4" /> Resolve Flag
                        </>
                      )}
                    </button>
                  </div>
