...
const VaultView = lazy(() =>
  import('./views/VaultView').then((module) => ({ default: module.VaultView })),
);
const MentorReviewView = lazy(() =>
  import('./views/MentorReviewView').then((module) => ({ default: module.MentorReviewView })),
);
const StudyRoomView = lazy(() =>
  import('./views/StudyRoomView').then((module) => ({ default: module.StudyRoomView })),
);
const LoginView = lazy(() => import('./views/LoginView').then((m) => ({ default: m.LoginView })));
const TrustDashboardView = lazy(() =>
  import('./views/TrustDashboardView').then((m) => ({ default: m.TrustDashboardView })),
);
const InterventionView = lazy(() =>
  import('./views/InterventionView').then((m) => ({ default: m.InterventionView })),
);
...
const WaitlistView = lazy(() =>
  import('./views/WaitlistView').then((m) => ({ default: m.WaitlistView })),
);
...
export default function App() {
  ...
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState<boolean>(() => {
    try {
      return localStorage.getItem('studyflow_sidebar_collapsed') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const toggleSidebar = () => {
    ...
    try {
      localStorage.setItem('studyflow_sidebar_collapsed', String(newVal));
    } catch (e) {}
  };
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:z-[999] focus:p-4 focus:bg-white focus:text-blue-600 focus:font-bold focus:shadow-xl focus:rounded-br-xl top-0 left-0"
      >
          ...
          <Route
            ...
            element={
              <>
                <SignedIn>
                  <div
                    className={`flex flex-col flex-1 min-h-0 h-full w-full transition-all duration-300 ${isSidebarCollapsed ? 'md:pl-[72px]' : 'md:pl-[240px]'}`}
                  >
                    <Header
                    ...
                    <SettingsModal
                      ...
                      onToggleTeacherMode={() => {
                        ...
                        try {
                          localStorage.setItem('studyflow_teacher_mode', String(newVal));
                        } catch (e) {}
                      }}
                    ...
                    <main
                      id="main-content"
                      className={`flex-1 min-h-0 w-full mx-auto relative overflow-x-hidden scroll-smooth ${location.pathname === '/chat' ? 'px-0 max-w-none overflow-y-hidden flex flex-col' : 'px-4 md:px-6 lg:px-8 max-w-[1600px] overflow-y-auto pt-4'}`}
                    >
                      <Breadcrumbs />
                                ...
                                element={
                                  <VaultView
                                    ...
                                    onSelectProblem={(prob) =>
                                      console.log('Selected problem:', prob.id)
                                    }
                                    soundEnabled={soundEnabled}
                                    onNotify={handleNotify}
                                  />
